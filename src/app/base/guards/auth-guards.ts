...
export const redirectAfterLogin: () => AuthPipe = () =>
	map((userData: User | null): boolean | string[] => {
		if (userData) {
			return userData.emailVerified ? TASK_BOARD_ROUTE : VERIFY_EMAIL_ROUTE;
		}
		return true;
	});
export const redirectNotAuthorized: () => AuthPipe = () =>
	map((userData: User | null): boolean | string[] => {
		if (userData) {
...
export const redirectVerified: () => AuthPipe = () =>
	map((userData: User | null): boolean | string[] => {
		if (userData) {
